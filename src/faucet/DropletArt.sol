...
/// @title The Droplet art storage contract
pragma solidity 0.8.20;

import { IDropletArt } from '../interfaces/IDropletArt.sol';
import { SSTORE2 } from '../libs/SSTORE2.sol';
import { IInflator } from '../interfaces/IInflator.sol';

contract DropletArt is IDropletArt {
     ...
     */
    function background(uint256 index) public view override returns (bytes memory) {
    ...
    function addPage(
        ...
        trait.storagePages.push(
            DropletArtStoragePage({ pointer: pointer, decompressedLength: decompressedLength, imageCount: imageCount })
        );
        trait.storedImagesCount += imageCount;
    }

    function imageByIndex(IDropletArt.Trait storage trait, uint256 index) internal view returns (bytes memory) {
        (IDropletArt.DropletArtStoragePage storage page, uint256 indexInPage) = getPage(trait.storagePages, index);
     ...
     */
    function getPage(IDropletArt.DropletArtStoragePage[] storage pages, uint256 index)
        internal
        view
        returns (IDropletArt.DropletArtStoragePage storage, uint256)
    {
        uint256 len = pages.length;
        uint256 pageFirstImageIndex = 0;
        for (uint256 i = 0; i < len; i++) {
            IDropletArt.DropletArtStoragePage storage page = pages[i];
    ...
    function decompressAndDecode(IDropletArt.DropletArtStoragePage storage page) internal view returns (bytes[] memory) {
