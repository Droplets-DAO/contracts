...
contract GDACurve is ICurve, CurveErrorCodes {
    ...
    uint256 internal constant _TIME_SCALAR = 2 * uUNIT_3; // Used in place of Euler's number
        ...
        )
    {
        ...
        UD60x18 decayFactor;
        {
            ...
            if (convert_0(exponent) > _MAX_TIME_EXPONENT) {
                // Cap the max decay factor to 2^10
                exponent = convert_1(_MAX_TIME_EXPONENT);
            }
            decayFactor = ud(_TIME_SCALAR).pow_1(exponent);
        }

        (UD60x18 alpha,,) = _parseDelta(delta);
        UD60x18 alphaPowN = alpha.powu_1(numItems);
        ...
        {
            UD60x18 newSpotPrice_ = spotPrice_.mul_1(alphaPowN);
            newSpotPrice_ = newSpotPrice_.div_1(decayFactor);
            if (newSpotPrice_.gt_1(ud(type(uint128).max))) {
                return (Error.SPOT_PRICE_OVERFLOW, 0, 0, 0, 0, 0);
            }
            if (newSpotPrice_.lt_1(ud(MIN_PRICE))) {
                return (Error.SPOT_PRICE_UNDERFLOW, 0, 0, 0, 0, 0);
            }
            newSpotPrice = uint128(unwrap_3(newSpotPrice_));
        }
        ...
        {
            UD60x18 inputValue_ = spotPrice_.mul_1(alphaPowN.sub_1(UNIT_4)).div_1(alpha.sub_1(UNIT_4)).div_1(decayFactor);

            // Account for the protocol fee, a flat percentage of the buy amount
            protocolFee = unwrap_3(inputValue_.mul_1(ud(protocolFeeMultiplier)));

            // Account for the trade fee, only for Trade pools
            tradeFee = unwrap_3(inputValue_.mul_1(ud(feeMultiplier)));

            // Add the protocol and trade fees to the required input amount and unwrap to uint256
            inputValue = unwrap_3(inputValue_.add_1(ud(protocolFee)).add_1(ud(tradeFee)));
        }
        ...
        )
    {
        ...
        UD60x18 boostFactor;
        {
            ...
            boostFactor = ud(_TIME_SCALAR).pow_1(exponent);
        }
        ...
        // implicitly stores the the initial price, total items sold so far, and time elapsed since the start.
        {
            UD60x18 newSpotPrice_ = spotPrice_.mul_1(boostFactor);
            newSpotPrice_ = newSpotPrice_.div_1(alphaPowN);
            if (newSpotPrice_.gt_1(ud(type(uint128).max))) {
                return (Error.SPOT_PRICE_OVERFLOW, 0, 0, 0, 0, 0);
            }
            if (newSpotPrice_.lt_1(ud(MIN_PRICE))) {
        ...
        UD60x18 outputValue_ =
            spotPrice_.mul_1(boostFactor).div_1(alphaPowN.div_1(alpha)).mul_1(alphaPowN.sub_1(UNIT_4)).div_1(alpha.sub_1(UNIT_4));

        // Account for the protocol fee, a flat percentage of the sell amount
        protocolFee = unwrap_3(outputValue_.mul_1(ud(protocolFeeMultiplier)));

        // Account for the trade fee, only for Trade pools
        tradeFee = unwrap_3(outputValue_.mul_1(ud(feeMultiplier)));

        // Remove the protocol and trade fees from the output amount and unwrap to uint256
        outputValue = unwrap_3(outputValue_.sub_1(ud(protocolFee)).sub_1(ud(tradeFee)));
